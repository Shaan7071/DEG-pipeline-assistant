...
# 
# 
# Version 6 is the final version of the pipeline that is accessed by the CLI.
# 
...
def create_boxplots(norm_file_name, conditions, num_replicates, base_dir):
    ...
    for i in conditions:
        ...
        plt.title('Boxplots of Normalized Counts Across Replicates for ' + conditions[set] + ' Condition')
        plt.xlabel('Samples')
...
def create_pca(norm_file_name, base_dir):
    ...
    plt.title('PCA of Gene Expression Data', fontsize=14)
    plt.grid(True)
...
def create_ma_plots(input_dir_pw_data, base_dir):
    ...
    for file_name in pw_file_names:
        ...
        plt.xlabel('A (Average log2 expression)', fontsize=12)
        plt.ylabel('M (Log2 fold change)', fontsize=12)
        plt.title(f'MA Plot: {os.path.basename(file_name).replace("_processed.csv", "")}', fontsize=14)
        
        # Add a legend
        plt.legend(['Not significant', 'Significant (padj<0.05, |log2FC|>1)', 'M=0'])
...
def create_volcano_plots(input_dir_pw_data, base_dir, log2fc_threshold, padj_threshold):
    ...
    for file_name in pw_file_names:
        ...
        # Customize the plot
        plt.title(f'Volcano Plot: {os.path.basename(file_name).replace("_processed.csv", "")}', fontsize=14)
...
def perform_GO_enrichment (pw_interest, base_dir, enrich_sig_cutoff):
    ...
    for file in pw_interest:
        ...
        if len(gene_list) > 0:
            ...
            bars = plt.barh(top10_terms['Term'], top10_terms['Gene Count'],
                            color=green_cmap(norm(top10_terms['Adjusted P-value'])))

            plt.xlabel('Gene Count')
            plt.ylabel('GO Term Description')
            plt.title(f'BP GO Terms: {comp_name}')
...
def perform_KEGG_enrichment (pw_interest, base_dir, enrich_sig_cutoff):
    ...
    for file in pw_interest:
        ...
        if len(gene_list) > 0:
            ...
            plt.xlabel('Gene Count')
            plt.ylabel('KEGG Term Description')
            plt.title(f'KEGG Terms: {comp_name}')
            plt.gca().invert_yaxis()
